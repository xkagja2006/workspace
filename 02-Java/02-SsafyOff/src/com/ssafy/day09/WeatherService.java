...
public class WeatherService extends DefaultHandler {
	public WeatherService() throws Throwable {
		getWeatherInfo("https://www.kma.go.kr/wid/queryDFSRSS.jsp?zone=3020052600");

		System.out.println("-----------------DOM파싱-----------------");
		List<Weather> list = domProcessing();
        for (Weather info : list) {
            System.out.println(info);
        }
        
        System.out.println("-----------------SAX파싱-----------------");
        WeatherSAX handler = new WeatherSAX();

        List<Weather> list2 = handler.getWeatherBySAX();
        for (Weather weather : list2) {
            System.out.println(weather);
        }
	}

	public List<Weather> domProcessing() throws Exception {
