...
//			}
		...
		// t1 승, t2 패
		if (result[t1][0] + 1 <= data[t1][0] && result[t2][2] + 1 <= data[t2][2]) {
			result[t1][0]++;
			result[t2][2]++;
			dfs(cnt + 1);
			// 이겼던 게임 원상복귀 시키기
		...
		// t1 무, t2 무
		if (result[t1][1] + 1 <= data[t1][1] && result[t2][1] + 1 <= data[t2][1]) {
			result[t1][1]++;
			result[t2][1]++;
			dfs(cnt + 1);
			result[t1][1]--;
		...
		// t1 패, t2 승
		if (result[t1][2] + 1 <= data[t1][2] && result[t2][0] + 1 <= data[t2][0]) {
		result[t1][2]++;
		result[t2][0]++;
		dfs(cnt + 1);
		result[t1][2]--;
