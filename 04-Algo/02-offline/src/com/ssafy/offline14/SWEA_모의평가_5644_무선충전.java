...
public class SWEA_모의평가_5644_무선충전 {
	static int M, A, ans;
	static class User {
		int r, c;
		User(int r, int c) {
	...
	public static void main(String[] args) throws Exception {
		...
		while (t++ < T) {
			...
			path1 = new int[M + 1];
			path2 = new int[M + 1];
			for (int i = 1; i <= M; i++) path1[i] = sc.nextInt();
			for (int i = 1; i <= M; i++) path2[i] = sc.nextInt();
			bc = new BC[A];
...
//				 int sum = 0;
				int aSum = getBCPerformance(a, u1);
				int bSum = getBCPerformance(b, u2);
				/*
